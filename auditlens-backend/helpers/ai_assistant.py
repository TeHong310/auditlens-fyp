...
_ACTION_INSTRUCTIONS = {
    ...
    'approval_assessment': (
        ...
        'Each of blocking_issues / passed_checks / recommended_next_'
        'steps MUST have AT MOST 4 items, each a short phrase or one '
        'short sentence — never a paragraph. Pick only the 3-4 MOST '
        'important points, not every possible one.\n'
        'blocking_issues: unresolved problems that actually prevent '
        'approval right now (a missing document, a mismatch, an '
        'authenticity concern, an unresolved risk finding) — empty only '
        'when this case is otherwise clean. A historical/already-'
        'reviewed finding is NOT a blocking issue.\n'
        ...
        'flagged risk finding. If nothing is blocking, the only step is '
        ...
        '"passed_checks": ["string", ...], "recommended_next_steps": '
        '["string", ...]}'
    ),
    'generate_remark': (
